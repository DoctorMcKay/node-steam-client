...
UDPConnection.prototype.connect = function(options) {
	...
	var ephemeral = Math.floor(Math.random() * (65535 - 49152)) + 49152;
	this.socket.bind(options.localPort || ephemeral, options.localAddress || "0.0.0.0");

	var self = this;

	this.socket.on('error', function(err) {
		if(err.code == 'EADDRINUSE' && !options.localPort) {
			// The ephemeral port we picked randomly is in use. Just try again.
