...
// Not only is this broken, it doesn't even belong in here as technically speaking
// it's not a 'client' operation... it's a 'user' operation. The client should only be responsible
// for low level ops like connection and messge send/receive.
/**
...
CMClient.prototype.send = function(header, body, callback) {
	...
	// only a few messages are valid when not logged on
	if (!this.loggedOn) {
		...
		switch(header.msg) {
			...
			case EMsg.ClientLogonGameServer:
				// upstream must set this.steamID before trying to logon
				this.steamID ? this._send(header, body, callback) : this.emit('error', new Error("ENOSTEAMID"));
				break;

			default:
				this.emit('error', new Error("ENOLOGON"));
